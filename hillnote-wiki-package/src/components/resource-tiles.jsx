...
// Helper to safely use config - returns null if not in context
...
export function ResourcePDF({ src, title, workspacePath }) {
  const config = useSafeConfig()
  const configWorkspacePath = config?.workspace?.path || workspacePath || ''
...
export function ResourceHTML({ src, title, workspacePath }) {
  const config = useSafeConfig()
  const configWorkspacePath = config?.workspace?.path || workspacePath || ''
  const handleOpen = () => {
    // Resolve the URL
...
export function ResourceFile({ path, name, type, workspacePath }) {
  const config = useSafeConfig()
  const configWorkspacePath = config?.workspace?.path || workspacePath || ''
