...
import { AuthorsNotes } from './authors-notes.jsx'
import { RelatedDocuments } from './related-documents.jsx'
import { useConfig } from './ConfigProvider'
import { cn } from '../lib/utils'

export function DocumentFooter({ comments, relatedDocuments, onDocumentClick }) {
  const config = useConfig()
  const [activeTab, setActiveTab] = useState('related')
  
  const showAuthorsNotes = config.ui.authorsNotes?.enabled && comments.length > 0
  const showRelated = config.ui.relatedDocuments?.enabled && relatedDocuments.length > 0
  ...
  if (showAuthorsNotes && !showRelated) {
    return (
      <div className="mt-12 pt-8">
        <h2 className="text-lg font-semibold mb-6">{config.ui.authorsNotes?.title || "Author's Notes"}</h2>
  ...
  if (showRelated && !showAuthorsNotes) {
    return (
      <div className="mt-12 pt-8">
        <h2 className="text-lg font-semibold mb-6">{config.ui.relatedDocuments?.title || "Related Documents"}</h2>
