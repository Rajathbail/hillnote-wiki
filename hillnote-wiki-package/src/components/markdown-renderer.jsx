...
import { ScratchSpace } from './scratchspace'
import { ResourcePDF, ResourceHTML, ResourceFile, ResourceYouTube } from './resource-tiles'
      ...
      try {
        ...
        try {
          ...
          if (registryResponse.ok) {
            ...
            let doc = registry.documents?.find((d) => d.path === filePath)
            ...
            if (!doc && filePath.includes('/')) {
              ...
              const candidates = registry.documents?.filter((d) => {
        ...
        if (config.ui.authorsNotes?.enabled) {
        ...
        if (config.ui.relatedDocuments?.enabled) {
          const docLinks = []
          ...
          if (fullRegistry) {
            ...
            fullRegistry.documents?.forEach((doc) => {
            ...
            const currentDoc = fullRegistry.documents?.find((d) => d.path === resolvedPath)
            ...
            if (currentDoc && currentDoc.referencedIn) {
              currentDoc.referencedIn.forEach((refPath) => {
                const refDoc = fullRegistry.documents?.find((d) => d.path === refPath)
          ...
          while ((match = linkRegex.exec(markdown)) !== null) {
            ...
            if (!docLinks.some(doc => doc.path === path)) {
              ...
              if (!docInfo && fullRegistry) {
                const foundDoc = fullRegistry.documents?.find((d) => 
                  d.path === path || 
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (contentRef.current) {
        ...
        pdfPlaceholders.forEach(placeholder => {
          ...
          const root = createRoot(container)
          root.render(<ResourcePDF src={src} title={title} workspacePath={config.workspace.path} />)
        ...
        htmlPlaceholders.forEach(placeholder => {
          ...
          const root = createRoot(container)
          root.render(<ResourceHTML src={src} title={title} workspacePath={config.workspace.path} />)
          resourceRootsRef.current.push(root)
        })
        ...
        filePlaceholders.forEach(placeholder => {
          ...
          const root = createRoot(container)
          root.render(<ResourceFile path={path} name={name} type={type} workspacePath={config.workspace.path} />)
          resourceRootsRef.current.push(root)
        })
